...
#include <signal.h>     // in order not to wait child
#define BUF_LEN 1024
...
int main()
{
    ...
    for(;;){
        ...
        // fork and not to wait child
        pid_t pid;
        signal(SIGCHLD, SIG_IGN);    // signal child , signal ignore,  call before fork
